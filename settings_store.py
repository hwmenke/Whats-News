...
Single-row table `app_settings` (id = 1). Used by the journal sizing form
...
def get_settings():
    ...
    row = conn.execute(
        "SELECT equity, risk_pct FROM app_settings WHERE id = 1"
    ).fetchone()
    conn.close()
    if not row:
...
def set_settings(equity=None, risk_pct=None):
    ...
    conn.execute(
        """
        INSERT INTO app_settings (id, equity, risk_pct, updated_at)
        VALUES (1, ?, ?, ?)
