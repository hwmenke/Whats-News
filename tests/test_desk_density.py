...
class DeskDensityTests(unittest.TestCase):
    ...
    def test_book_surfaces_split(self):
        ...
        self.assertIn("_navTick", dart)
        self.assertIn("DeskSpace.headerContent", dart)
        self.assertIn("DeskSpace.inset", dart)
        fill = dart[dart.find("Widget _pnlFill"):dart.find("List<Widget> _riskSlivers")]
        self.assertIn("EdgeInsets.fromLTRB", fill)
        self.assertNotIn("fromLTRB(20, 8, 20, 24)", dart)
        self.assertNotIn("fontSize: 64", dart)
        self.assertIn("Expanded(", fill)
        self.assertIn("('Equities'", fill)
        self.assertNotIn("SingleChildScrollView(", fill)
        self.assertNotIn("class _CurvePainter", dart)
