...
def init_db():
    ...
    # ── Trade journal (momentum-process risk journal) ───────────────────────────
    cur.execute("""
    ...
    # ── Journal settings (single-row: equity / risk % for position sizing) ──────
    # Named distinctly from app_settings (a generic KV store used elsewhere).
    cur.execute("""
